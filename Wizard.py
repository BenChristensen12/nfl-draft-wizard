    ...
    for i, tab in enumerate(tabs):
        with tab:
            if i == 0:
                ...
                data_df = st.session_state.data.sort_values(["Taken", "draft_position"]).loc[st.session_state.data.position.isin(["WR", "RB", "TE"]), ["Taken", "Player", "projection", "lead", "at_risk", "outlook"]].round()
                lead = data_df.projection.max() - data_df.loc[~data_df.at_risk, "projection"].max()
                data_df["lead"] = None
                data_df.sort_values("projection", ascending = False, inplace = True)
